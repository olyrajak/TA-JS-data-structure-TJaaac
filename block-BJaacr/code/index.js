...
for (let i = 0; i < got.houses[1].people.length; i++) {
  console.log(got.houses[1].people[i].name, " - Starks");
}
...
for (let i = 0; i < got.houses[0].people.length; i++) {
  if(got.houses[0].people[i].name==='Robb Stark'){
    console.log("The Name" ,got.houses[0].people[i].name, "and Description " ,got.houses[1].people[i].description);

  }
}
// // 10. Log the name and description of the people in house Lannisters whose name is `Tywin Lannister`
for (let i = 0; i < got.houses[0].people.length; i++) {
  if(got.houses[0].people[i].name==='Tywin Lannister'){
    console.log("The Name" ,got.houses[0].people[i].name, "and Description " ,got.houses[1].people[i].description);
...
console.log(got.houses[1].people.length, "members in House of Starks");
...
for (let j = 0; j < got.houses.length; j++) {
  for (let i = 0; i < got.houses[j].people.length; i++) {
   if(got.houses[j].people[i].description.length>30){
    console.log(got.houses[j].people[i].description);
   }
  }
}